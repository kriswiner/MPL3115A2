...
const int int2Pin = 5;  
// This pin set momentarily HIGH starts FIFO mode with whatever auto time step has been set above;
// Later we will allow on board setting of time step;
// When FIFO mode count hits overflow/watermark (currently set to 32), a second momentary setting of this pin to HIGH
// burst reads the data and prints it to the serial monitor. After read, device returns to real-time mode.
// Autonomous data logging can be invoked more than once.
const int readoutPin = 2; 
...
// Currntly configured to start with real-time data display to serial monitor and/or LCD display. When pin 2 is momentarity set HIGH,
// the device enters the autonomous FIFO mode and collects data until the overflow/watermark flag is reached. A second setting of pin 2
// to momentaroty HIGH burst reads the data to the serial monitor. After read, the device returns to the real-time data acquisition mode.
// I have added a 1 kOhm resistor between pin 2 and ground to ensure proper return to the LOW condition after pin 2 is set HIGH by
// the push button. This is a simple pull-down feature. Also, when autonomous data logging, it is necessary to keep the reset high while
// the device is reconnected to the FTDI and a new serial monitor is opened for data reading. These two actions will pull the reset low,
// causing the device to reset and the data to be lost. A 22 uF capcitor between reset and ground prevents this when reconnecting.

  if(digitalRead(readoutPin) == HIGH) { // Enter FIFO mode if this pin is momentarily HIGH, otherwise display real-time data
...
//
   ...
   for (i=0; i <= 30*count; i++) {
     ...
     lcd.setCursor(6,0); lcd.print((30*count - i)); lcd.print("s    "); 
...
// and downloaded to the seial monitor or other output device at another time. This is a poor man's data logger!
   while(digitalRead(readoutPin) == LOW); // Wait for the readout pin to go momentarily HIGH again before data read
